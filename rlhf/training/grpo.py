...
Judge calls are dispatched in parallel via a persistent thread pool so the
GPU doesn't idle waiting on serial HTTP round-trips.
...
def _parse_score(raw: str) -> float:
    """Parse a judge response into a 1-5 score, with a regex fallback."""
    raw = raw.strip()
    try:
        return float(raw)
    except ValueError:
...
def judge_completion(
    ...
    """Score a single (diff, comment) pair with the Claude judge.

    Retries on transient API errors with exponential backoff. Returns a
    neutral 1.0 on terminal failure rather than crashing training.
    """
    ...
    for attempt in range(max_attempts):
        try:
            ...
            score = _parse_score(response.content[0].text)
            if judge_sleep:
            ...
            wait = 2**attempt * 5
            print(f"\n  [judge] rate limit — waiting {wait}s...")
            time.sleep(wait)
            ...
            wait = 2**attempt * 2
            print(f"\n  [judge] transient error ({type(e).__name__}) — waiting {wait}s...")
            time.sleep(wait)

        except (IndexError, anthropic.BadRequestError):
            # Malformed response or unfixable request; don't retry.
            return 1.0
...
def make_reward_fn(judge_model: str, judge_sleep: float, max_workers: int = 8):
    """Build a TRL-compatible reward_fn that fans out judge calls in parallel.

    The thread pool is created once and reused across training steps. The
    Anthropic sync client is thread-safe (httpx under the hood), so a single
    client instance is shared across workers.
    """
    try:
    ...
    # Persistent pool: avoids per-step thread churn. Daemon threads are
    ...
    def reward_fn(completions, prompts, **kwargs):
        # submit() preserves submission order in the returned list, so we
        # keep reward ordering aligned with the (prompt, completion) pairs
        # TRL handed us. Don't use as_completed() here.
        jobs = [
            executor.submit(
                judge_completion, p, c, client, judge_model, judge_sleep
            )
...
def main() -> None:
    ...
    # Pick a sensible default if max_workers is absent from config.
    max_workers = int(judge_cfg.get("max_workers", 8))
    ...
    print(f"  Judge:       {judge_cfg['model']} ({max_workers} parallel workers)")
...
if __name__ == "__main__":
    main()"""GRPO post-training for the code review LoRA adapter.
